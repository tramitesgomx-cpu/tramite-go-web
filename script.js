...
// CORS abierto, acepta servicio/tramite/nombreServicio,
// whatsapp/telefono/phone, correo/email
// =====================================================
async function sincronizarLead(lead) {
  ...
  // Construir payload con los nombres EXACTOS que acepta el endpoint
  const payload = {
    nombre: lead.nombre || '',
  ...
  try {
    ...
    if (res.ok) {
      ...
      console.warn('⚠️ Sistema interno respondió con error:', res.status);
      // Reintentar con sendBeacon como respaldo
      if (navigator.sendBeacon) {
        const blob = new Blob([JSON.stringify(payload)], { type: 'application/json' });
        navigator.sendBeacon(ENDPOINT, blob);
        console.log('📡 Reintentado via sendBeacon');
      }
      return { success: false, status: res.status };
    }
  } catch (err) {
    console.warn('⚠️ Error de red al registrar lead:', err.message);
    // Si fetch falla por CORS u otro motivo, intentar con sendBeacon
    if (navigator.sendBeacon) {
      try {
        const blob = new Blob([JSON.stringify(payload)], { type: 'application/json' });
        navigator.sendBeacon(ENDPOINT, blob);
        console.log('📡 Enviado via sendBeacon (respaldo)');
...
function construirMensajeLead(lead) {
  ...
  if (lead.respuestas && Object.keys(lead.respuestas).length > 0) {
    partes.push('Respuestas del evaluador: ' + Object.values(lead.respuestas).join(' | '));
  }
  if (lead.urgencia && lead.urgencia !== 'pronto') {
    partes.push('Urgencia: ' + lead.urgencia);
  }
  if (lead.ciudad) partes.push('Ciudad: ' + lead.ciudad);
  if (lead.serviciosLista) partes.push('Servicios solicitados: ' + lead.serviciosLista);
